...
/**
 * Renders security-filter rejections (deny-by-default, fail-closed metrics) with the
 * same ProblemDetail contract as the MVC advice (AGENTS.md §6, NFR-06, ADR-007):
 * 401 {@code urn:foley-books:problem:unauthenticated} from the entry point and 403
 * {@code urn:foley-books:problem:forbidden} from the denied handler, each carrying
 * {@code instance} and a {@code traceId} that correlates with the server logs.
 */
@Component
