 ...
 *
 * <p>Tokens are opaque 256-bit values ({@link OpaqueTokens#randomValue()} —
 * 32 {@link SecureRandom} bytes rendered as 64 lowercase hex chars, URL-safe
 * for the confirmation link). Only {@link OpaqueTokens#sha256Hex} — also 64
 * lowercase hex chars — is persisted; the
...
public class ConfirmationTokenServiceImpl implements ConfirmationTokenService {
    ...
    public Optional<ConfirmationToken> find(String rawToken) {
        ...
        return confirmationTokenRepository.findByTokenHash(OpaqueTokens.sha256Hex(rawToken));
    }
    ...
    private String reissue(User user, boolean stampResend) {
        ...
        String tokenHash = OpaqueTokens.sha256Hex(rawToken);
