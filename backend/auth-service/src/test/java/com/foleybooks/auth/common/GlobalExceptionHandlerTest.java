...
class GlobalExceptionHandlerTest {
    ...
    private final MockHttpServletRequest servletRequest = new MockHttpServletRequest("POST", "/api/v1/auth/register");
    private WebRequest request;

    @BeforeEach
    void setUp() {
        servletRequest.setRequestURI("/api/v1/auth/register");
    ...
    void handleMethodArgumentNotValid_whenBodyFieldsInvalid_returnsValidationProblemWithErrors() {
        ...
        bindingResult.rejectValue("password", "Pattern.probeRequest.password", "must contain at least one letter and one digit");
        bindingResult.rejectValue("email", "NotBlank.probeRequest.email", "must not be blank");
        ...
        assertThat(problem.getInstance()).isEqualTo(URI.create("/api/v1/auth/register"));
        assertThat(traceIdOf(problem)).matches("[0-9a-f]{8}");
        assertThat(errorsOf(problem))
                .containsExactly(
                        new ValidationError("password", "must contain at least one letter and one digit"),
                        new ValidationError("email", "must not be blank"));
    }
    ...
    void handleHandlerMethodValidationException_whenParameterConstraintFails_returnsErrorForThatParameter() {
        ...
        var exception = new HandlerMethodValidationException(
                MethodValidationResult.create(new ProbeRequest("a@b.com", "pass1234"), VALIDATE_METHOD,
    ...
    void handleMissingServletRequestParameter_whenParameterAbsent_returnsRequiredErrorForParameter() {
        var exception = new MissingServletRequestParameterException("token", "string");
        ...
        assertThat(problemStatus(response)).isEqualTo(400);
        assertThat(errorsOf(problemOf(response)))
                .containsExactly(new ValidationError("token", "is required"));
    }
    ...
    void handleConstraintViolation_whenViolationsPresent_returnsErrorsSortedByPath() {
        var exception = new ConstraintViolationException(Set.of(
                violation("password", "must contain at least one letter and one digit"),
    ...
    void handleHttpMessageNotReadable_whenBodyMalformed_returnsGenericBodyProblemWithoutParserDetails() {
        var exception = new HttpMessageNotReadableException(
                "JSON parse error: unexpected token near password hunter2", (HttpInputMessage) null);
    ...
    void handleUnexpected_whenUnknownExceptionThrown_returnsGeneric500WithoutLeakingCause() {
        var exception = new IllegalStateException("connect refused to auth-db with password=hunter2");
        ...
        assertThat(problem.getDetail()).doesNotContain("hunter2").doesNotContain("auth-db");
        assertThat(traceIdOf(problem)).matches("[0-9a-f]{8}");
    }
    ...
    record ProbeRequest(String email, String password) {
    }
