...
/**
 * The single error surface of order-service (D-15, NFR-06): every failure — bean
 * validation, malformed requests, {@link ApiException} business rules (the future
 * 404/422 cart statuses), {@link FeignException} catalog-outage propagation and
 * unexpected server errors — is rendered as an RFC 7807 ProblemDetail with a
 * {@code urn:foley-books:problem:*} type URN, the request path as {@code instance}
 * and a {@code traceId} that correlates the response with the server logs. 400s
 * additionally carry an {@code errors[]} array of {@link ValidationError}. Unexpected
 * failures return a generic message; the real cause is only ever logged.
 */
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    static final String CATALOG_UNAVAILABLE_TYPE = ApiException.TYPE_URN_PREFIX + "catalog-unavailable";
    ...
    /**
     * A catalog that is down, timing out or 5xx-ing surfaces as a generic, traceable
     * dependency error — never laundered into {@code available=false} cart flags
     * (ADR-005: "absence is data; unavailability is an error", NFR-07). 503 with a
     * dependency URN is the ADR-005-sanctioned mapping; the upstream status and body
     * stay out of the response (they may echo request data, C24). The specific
     * 404-to-BookNotFoundException decode is OR-05's ErrorDecoder job and never
     * reaches this handler.
     */
    @ExceptionHandler(FeignException.class)
    public ResponseEntity<ProblemDetail> handleFeignFailure(FeignException ex, HttpServletRequest request) {
        String traceId = TraceIdFilter.currentTraceId();
        // Only the upstream HTTP status is logged — FeignException messages embed response bodies.
        log.error("Catalog dependency call failed [{}] on {} {} (upstream HTTP {})",
                traceId, request.getMethod(), request.getRequestURI(), ex.status());
        ProblemDetail problem = problem(HttpStatus.SERVICE_UNAVAILABLE, CATALOG_UNAVAILABLE_TYPE,
                "Service temporarily unavailable",
                "The catalog service could not be reached. Please try again later.", request);
        return problemResponse(problem);
    }
